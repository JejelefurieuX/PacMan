...
export const IntroScreen: React.FC<IntroProps> = ({ gameState, startGame }) => {
  ...
  // Check if on mobile and not installed (PWA check)
  const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
  const isStandalone = window.matchMedia('(display-mode: standalone)').matches || (window.navigator as any).standalone;
  const showInstallHint = isMobile && !isStandalone;
    ...
    >
      ...
      */}
      ...
      <div className="absolute bottom-20 w-full text-center z-20 pointer-events-none">
        <div className="text-white text-2xl md:text-4xl animate-[blink_1s_steps(2)_infinite] neon-text font-bold tracking-wider drop-shadow-lg">
            TAP TO START
        </div>
        <div className="text-gray-500 text-xs mt-4 tracking-[0.3em]">FREE PLAY • 1 CREDIT</div>
        
        {showInstallHint && (
             <div className="mt-6 text-cyan-400 text-xs animate-pulse font-bold bg-black/50 py-2 px-4 rounded-full inline-block border border-cyan-400/30">
